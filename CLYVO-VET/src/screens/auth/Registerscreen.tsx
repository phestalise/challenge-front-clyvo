...
const STEPS = [
  { key: "personal", label: "Seus dados", icon: "person-outline" as const },
  { key: "security", label: "Segurança", icon: "lock-closed-outline" as const },
];
...
export default function RegisterScreen({ navigation }: Props) {
  ...
  const handleRegister = async () => {
    const e = validarFormularioUsuario(form);
    if (Object.keys(e).length) { setErrors(e); return; }
    setLoading(true);
    try {
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        >
          ...
          <View style={styles.header}>
            <TouchableOpacity
              onPress={() => (step === 1 ? (animateStep(), setStep(0)) : navigation.goBack())}
              style={styles.back}
              activeOpacity={0.7}
            >
          ...
          <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
            ...
            <Text style={styles.sub}>Comece a cuidar do seu pet{"\n"}com inteligência hoje</Text>
          </Animated.View>
              ...
              <View key={s.key} style={styles.stepItem}>
                <View style={[styles.stepDot, i <= step && styles.stepDotActive, i < step && styles.stepDotDone]}>
                  {i < step ? (
                    ...
                    <Ionicons name={s.icon} size={14} color={i <= step ? Colors.primary : "rgba(255,255,255,0.35)"} />
                  )}
                </View>
          ...
          <Animated.View style={[styles.formCard, { opacity: fadeAnim, transform: [{ translateY: stepAnim }] }]}>
            {step === 0 ? (
              <>
              ...
              <>
                ...
                <View style={styles.passwordHint}>
                  ...
                  <Text style={styles.passwordHintText}>Use pelo menos 6 caracteres com letras e números</Text>
                </View>
              </>
            )}
          </Animated.View>
          ...
          <View style={styles.actions}>
            {step === 0 ? (
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <Text style={styles.btnPrimaryText}>{loading ? "Criando conta..." : "Criar conta"}</Text>
                {!loading && (
                  <View style={styles.btnArrow}>
            ...
            <View style={styles.legalRow}>
              <Ionicons name="shield-outline" size={11} color="rgba(255,255,255,0.25)" />
              <Text style={styles.legalText}>Ao criar conta, você aceita os Termos de Uso</Text>
            </View>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
